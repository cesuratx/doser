...
fn sampler_can_be_created_dropped_and_recreated() {
    ...
    let sampler1 = Sampler::spawn(scale1, 10, Duration::from_millis(50), clock);
    std::thread::sleep(Duration::from_millis(50));
    drop(sampler1);

    // Create another one - should not conflict
    let scale2 = NoopScale;
    let sampler2 = Sampler::spawn(scale2, 10, Duration::from_millis(50), clock);
    std::thread::sleep(Duration::from_millis(50));
    drop(sampler2);
...
fn sampler_shutdown_is_prompt() {
    ...
    assert!(
        ...
        "Shutdown took {shutdown_time:?}, expected < 200ms for prompt response"
    );
